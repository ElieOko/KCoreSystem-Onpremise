package com.schoolstats.presentation.screens.statistics
...
import com.schoolstats.presentation.components.CountField
...
private fun SummaryStrip(
    ...
    AdaptiveKpiGrid(
        compact,
        { KpiCard("Élèves inscrits", formatInt(boys + girls), "G ${formatInt(boys)} · F ${formatInt(girls)}", modifier = it) },
        { KpiCard("Enseignants", formatInt(teachers.sumOf { it.totalCount }), "H ${teachers.sumOf { it.menCount }} · F ${teachers.sumOf { it.womenCount }}", modifier = it) },
        { KpiCard("Administratif", formatInt(admin.sumOf { it.totalCount }), "Direction, préfet, secrétariat…", modifier = it) },
        { KpiCard("Ouvriers", formatInt(workers.sumOf { it.totalCount }), "H ${workers.sumOf { it.menCount }} · F ${workers.sumOf { it.womenCount }}", modifier = it) },
    )
}
