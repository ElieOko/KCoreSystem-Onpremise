package com.schoolstats.presentation.screens.centralization
...
@Composable
fun CentralizationScreen(
    compact: Boolean = false,
    viewModel: CentralizationViewModel = koinViewModel(),
) {
    ...
    Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        AdaptiveKpiGrid(
            ...
            { KpiCard("Enseignants", formatInt(stats.totalTeachers), "H ${formatInt(stats.totalTeacherMen)} · F ${formatInt(stats.totalTeacherWomen)}", ChartPalette.teal, it) },
            { KpiCard("Administratif", formatInt(stats.totalAdminStaff), "H ${formatInt(stats.totalAdminMen)} · F ${formatInt(stats.totalAdminWomen)}", ChartPalette.women, it) },
            { KpiCard("Ouvriers", formatInt(stats.totalWorkers), "H ${formatInt(stats.totalWorkerMen)} · F ${formatInt(stats.totalWorkerWomen)}", ChartPalette.gold, it) },
        )
        AdaptiveChartStack(
            compact,
            { mod ->
